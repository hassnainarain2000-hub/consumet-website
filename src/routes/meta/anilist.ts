  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (genres) {
        ...
        const invalidGenre = parsedGenres.find((genre: string) => !Object.values(Genres).includes(genre as Genres));
        if (invalidGenre) {
          return reply.status(400).send({ message: `${invalidGenre} is not a valid genre` });
        }
        genres = parsedGenres;
      }
